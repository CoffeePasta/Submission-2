...
public class AchievementController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (var achieve in _achievementList)
        {
            if(achieve.Type == AchievementType.GoldQuantity)
            {
                success = double.TryParse(achieve.Value, out double result);

                if (!success)
                    Debug.LogError("Wrong value set at " + achieve.Title + " in Achievement List");
            }
        }
#endif
    }
    ...
    public void UnlockAchievement(AchievementType type, double value)
    {
        // Mencari data achievement
        AchievementData achievement = _achievementList.Find(a => a.Type == type && a.IsUnlocked == false && double.Parse(a.Value) <= value);
